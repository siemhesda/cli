...
t.cleanSnapshot = s => {
  return s.replace(/"documentNamespace":".+?"/g, `"documentNamespace":"docns"`)
    .replace(/"created":".+?"/g, '"created":"2020-01-01T00:00:00.000Z"')
}
...
t.test('node - with deps', t => {
  ...
  const res = spdxOutput({ npm, nodes: [node, dep1, dep2, dep3, dep4] })
  t.matchSnapshot(JSON.stringify(res))
  t.end()
})
