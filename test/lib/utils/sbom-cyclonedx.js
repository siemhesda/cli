...
t.cleanSnapshot = s => {
  return s.replace(/"serialNumber":".+?"/g, `"serialNumber":"${FAKE_UUID}"`)
    .replace(/"timestamp":".+?"/g, '"timestamp":"2020-01-01T00:00:00.000Z"')
}
...
t.test('schema validation', t => {
  // Load schemas
  const cdxSchema = require('../../schemas/cyclonedx/bom-1.4.schema.json')
  ...
  const validate = ajv.getSchema('http://cyclonedx.org/schema/bom-1.4.schema.json')
